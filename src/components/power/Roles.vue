            ...
            <el-table :data="rolelist" border stripe>
                ...
                <el-table-column type="expand" label="权限">
                  ...
                  <template slot-scope="scope">
                    <!-- eslint-disable-next-line -->
                    <el-row :class="['vcenter', 'bdbottom',i1 === 0 ? 'bdtop' : '']" v-for="(item1,i1) in scope.row.children" v-bind:key="item1.id">
                      <!-- 渲染一级权限 -->
                      <el-col :span="5">
...
export default {
  ...
  methods: {
    ...
    // 通过递归的方式 获取角色的三级权限项 并保存到defKeys中
    getLeafKeys (node, arr) {
      if (!node.children) { // 是三级权限
